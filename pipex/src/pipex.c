...
/*   Updated: 2023/05/22 16:15:30 by iortega-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	call_child1(t_pipex pipex, char **argv, char **envp)
{
	dup2(pipex.port[1], 1);
	close(pipex.port[0]);
	dup2(pipex.infile, 0);
...
void	call_child2(t_pipex pipex, char **argv, char **envp)
{
	dup2(pipex.port[0], 0);
	close(pipex.port[1]);
...
int	main(int argc, char **argv, char **envp)
{
	...
	pipex.child1 = fork();
	if (pipex.child1 == 0)
		call_child1(pipex, argv, envp);
	pipex.child2 = fork();
	if (pipex.child2 == 0)
		call_child2(pipex, argv, envp);
	close_pipes(&pipex);
